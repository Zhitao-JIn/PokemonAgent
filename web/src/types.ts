...
/**
 * GET /runs/{id}/metrics 的一条链路（`Source`）聚合结果——对应后端
 * `evaluation/eval_report.py::SourceMetrics`（`docs/ROADMAP.md` 第 2 条
 * "可观测"第二拍）。`verify_*` 四个字段只对 `verify` 这条链路有意义，
 * 其余链路恒为 0/null。
 */
export interface SourceMetrics {
  ...
  tokens_in: number;
  tokens_out: number;
  /** 0905 新增：input_tokens 里命中 DashScope/火山方舟隐式缓存的部分，
   * 只按标准输入价 20% 计费。缺字段的老事件按 0 算，不进 malformed。 */
  ...
  verify_total: number;
  verify_unreliable: number;
  verify_unreliable_rate: number | null;
  verify_parse_errors: number;
}

/** GET /runs/{id}/metrics 里 run 级汇总的一行（对应后端 `RunSummary`）。 */
...
/** GET /runs/{id}/metrics 的完整响应——按 `Source` 聚合 + run 级汇总。 */
