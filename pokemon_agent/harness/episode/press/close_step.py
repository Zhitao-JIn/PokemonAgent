...
改 `observation`/`step` 两处（"一步结束了"这一个判定的两面）+ 预算闸可能清
`pending_presses`，并落一条 `STEP_ADVANCE`。
...
def close_step(state: EpisodeRunState, runtime: Runtime[HarnessDeps]) -> dict[str, Any]:
    ...
    后置条件：返回 `{"observation": …, "step": …}`；步号到上限时**清空
    `pending_presses`**（预算闸）。
    """
    ...
    deps.trace.append(
        FromHarnessToTraceToolAppendReq(
            ...
            meta={"source": "close_step", "episode_id": state.episode_id, "step": state.step},
    ...
    # **预算闸**（0914 跟进，截断判定删除后唯一的队列干预点）：步号到上限时，
    # 队列里还没按的键一律不按——链内小循环（回 `act`）不经过 judge，预算不在
    # 这一格执行就没有任何格子拦得住。**纯步数、零判断**：不看撞没撞墙、不看
    # 按的是什么，只看数字。模型交出的链在预算之内照单全按，到线即止。
    if new_state["step"] >= state.task.max_steps and state.pending_presses:
        new_state["pending_presses"] = []
    return new_state
