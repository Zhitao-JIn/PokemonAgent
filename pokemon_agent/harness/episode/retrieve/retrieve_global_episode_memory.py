...
def retrieve_global_episode_memory(
    ...
    **只改 `global_episode_memories` 一处。**

    前置条件：`state.observation` 非空。
    后置条件：返回 `{"global_episode_memories": …}`，并记一条 `read_global`
    （**空清单**与"没查"是两回事）。
    """
    ...
    episode_memories = deps.memory.query_episode_summaries(
        FromHarnessToMemoryToolQueryEpisodeSummariesReq(conditions={"run_id": deps.run_id})
    ).summaries
    deps.trace.append(
        FromHarnessToTraceToolAppendReq(
            kind=TraceKind.READ_GLOBAL,
            meta={"source": "retrieve_global_episode_memory", "episode_id": ep, "step": step},
            # 检索条件就是那条 `conditions`（本格唯一的一个等值项）。
            query=f"run_id={deps.run_id}",
