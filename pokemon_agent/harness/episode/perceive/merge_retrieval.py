...
`global_episode_memories`（跨局摘要）、`task_memories`（本局 TaskMemory）各自留在 `ep_ctx`
里，由判定与拆解分别取用，**不混进"已知事实"**。
...
折进去的东西去哪了：`obs.facts.known_objects` 随这一帧进 `plan_episode` 的
...
def merge_retrieval(state: EpisodeRunState, runtime: Runtime[EpisodeRuntime]) -> dict[str, Any]:
    """把语义 object 折进这一帧观测。**只改 `observation` 一处，不查库、不记
    trace。**

    前置条件：四路 retrieve 都已跑过（`state` 里四个字段齐备）、`state.ep_ctx` 非空。
    后置条件：返回 `{"observation": …}`；`knowledge`/`episode_memories` 原样留在 state 里
    给 `plan_episode` 自己取。
    """
    obs = state.ep_ctx.observation

    # 折进语义记忆（object）——坐标锚定，跟 walk_map/landmarks 同一可信度级别。
    # 知识库/跨局摘要**不**折进 obs.facts：真正喂给 plan_episode 的路径是 state
    # 里那两个字段原样传下去，由 plan_episode 自己拼
    ...
    if state.ep_ctx.object_semantic_memory:
        obs = obs.model_copy(
            update={
                "facts": obs.facts.model_copy(
                    update={"known_objects": state.ep_ctx.object_semantic_memory}
                )
            }
        )
