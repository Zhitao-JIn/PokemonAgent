...
放进任何一个域都会让别的域反过来 import 它。所以按"共用件"单独立一个模块：
`PLAN_graph_composition.md` §3.1 的树只列节点文件，没有为它安排位置——这段注释就是
它的位置说明。
...
def frame_ledger(
    ...
    """把本局的帧账摊成两张"步号说话"的表（按局切片取用）。

    两张表是纯内存态（`deps.frame_event_ids` / `deps.pending_frames`），供任何
    需要"按局看帧账"的场景切片；截图本来就在磁盘上
    （`screenshot/<event_id>.png`），这里只回答"哪条事件承载这一步这一帧"。

    只带**本局切片**：调用方要的总是"这一局"的帧账，别的局的帧账与它无关。

    前置条件：`episode_id` 非空。
    后置条件：返回 `(登记表, 暂存表)`，键是**整数步号**。
    """
