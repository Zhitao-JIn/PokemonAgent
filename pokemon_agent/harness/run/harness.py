...
`harness/console_planner.py`。

**依赖只有一个入口：`deps`**（步 4 收口）：原先构造参数里那些
`trace` / `brain_tool` / `reviewer` / 两个开关，全都已经是 `HarnessDeps` 的字段
...
from pokemon_agent.schemas.harness import FromRunHarnessToEpisodeHarnessRunResp, TraceEvent
...
class RunHarness:
    ...
    def __init__(self, deps: HarnessDeps) -> None:
        ...
        `deps.reviewer` / `deps.planner` **必须由装配处给全**（`build.py` 的缺省是
        `NullReviewer` / `BrainPlanner`）——本类不再兜底新建：那两个对象要
        **跨节点共享同一份策略**（`plan` 与 `review` 都得是同一个 `reviewer`），
        ...
        assert deps.reviewer is not None, "HarnessDeps.reviewer 必须由装配处给全"
        assert deps.planner is not None, "HarnessDeps.planner 必须由装配处给全"
        self.deps = deps
    ...
    def _compile(self) -> CompiledStateGraph:
        ...
        拓扑（`plan → dispatch → episode → review`，含两条条件边的判据、
        `START` 的入口，以及挂在 `episode` 那格上的 `error_handler`）全在
        `run/run_graph.py`；它不收 nodes 参数（节点就是 `nodes/` 下的 5 个模块）。
        """
