...
`dispatch` 拆出"接子图"的 `episode` 节点；步 4 把节点搬成自由函数、`RunHarness`
收成薄类（`harness.py`）、图外侧门落 `run_entry.py`；**0914 控制台改造删掉
`reflect`，节点从 6 格收成 5 格**。

**包根只放骨架，节点统一住 `nodes/`**：
run 图只有 5 格、没有 episode 那种"七个功能域"可分，所以不按域切，而是用一层不分域的
...
- 节点在 `nodes/`，**文件名 = 节点名**（§5.3-⑤）：`begin.py` / `plan.py` /
  `dispatch.py` / `episode.py` / `review.py`。
"""
