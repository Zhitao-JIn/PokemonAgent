...
**criteria 不能是残句。** 曾经有一条写的是 `"x或y坐标"`，judge 拿它无法比对
任何东西，只好回退去照 goal 的字面找证据——成功率恒为 0，而且看不出原因。
...
`facts` 是按 scene 装的（见 `observation.py` 的 `SCENE_FIELDS`）：
`my_hp` / `foe_hp` 只在 `scene=battle` 有，`money` / `items` 只在 `scene=shop` 有。
判据里写一个当前 scene 根本不会出现的字段，等于没写。

`pokemon_center_heal` 曾经写「或 `my_hp` 回到满值」——治疗发生在 `scene=indoor`，
那里永远读不到 `my_hp`。另外 `facts` 是**扁平的**，键就是 `money`、`items`，
没有 `fields` 这一层，判据里也不要这么写。
...
def knowledge_recall_tasks(max_steps: int = 15) -> list[TaskChain]:
    ...
    available_state_tasks = {
        ...
        "knowledge_shop_interaction", "knowledge_shop_open_buy_menu",
