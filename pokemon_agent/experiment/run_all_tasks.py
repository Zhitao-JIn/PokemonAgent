...
from .tasks import knowledge_recall_tasks
...
def main() -> None:
    ...
    chains = [chain for chain in knowledge_recall_tasks(args.max_steps) if len(chain.tasks) == 1]
    if args.only:
        wanted = {
            name if name.startswith("knowledge_") else f"knowledge_{name}" for name in args.only
        }
    ...
    print(
        f"[BATCH] {batch_id}：{len(chains)} 条任务 × {args.repeat} 遍 "
    ...
    for index, chain in enumerate(chains, start=1):
        ...
        completed = subprocess.run(
            [
                sys.executable,
                "-m",
                "pokemon_agent.experiment.run_experiment",
                "--task-id",
                chain.chain_id,
                "--repeat",
                str(args.repeat),
        ...
        results.append(
            (chain.chain_id, read_outcomes(chain.chain_id, run_ids), completed.returncode)
        )
