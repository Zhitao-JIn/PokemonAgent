...
这两个裸字段，从来不需要知道 `ActMemory`/`EpisodeMemory`/`ObjectFactEvent`
...
`harness/episode/store/store_object_semantic_memory/rules.py`）真正认识这几个类的字段。这正是
...
**`ActMemory`/`ObjectFactEvent` 内部对 `Observation`/`PlaceInWorld`
"形状像但类不同"的字段（`ActMemory.Observation`、
...
`harness/episode/store/store_object_semantic_memory/rules.py`）负责把真身
