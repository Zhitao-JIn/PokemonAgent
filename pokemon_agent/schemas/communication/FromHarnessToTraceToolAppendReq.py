...
class FromHarnessToTraceToolAppendReq(BaseModel):
    ...
    # ---- 边界 / 结算 ----
    task: TaskForHarness | None = None
    run_goals: list[TaskForHarness] | None = None
    """run 级边界的初始目标栈（`observe` 的 goals 是 GoalForBrain，
    两处词表不同，各用各的字段）。"""
    memory_carried: int | None = None
    outcome_run: RunOutcomeResp | None = None
    outcome_episode: HarnessEpisodeOutcomeResp | None = None
