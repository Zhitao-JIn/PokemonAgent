...
def build_prompt(req: ChooseOnceReq) -> str:
    ...
    **只读 `req.prompt` 之外的字段**——`req.prompt` 是这次调用要回填的输出，
    不是输入；调用方通常这样用：`prompt = build_prompt(req)` 再
    `req = req.model_copy(update={"prompt": prompt})`，同一个 `req` 对象
    随后交给 `Brain.choose_once(req)`。
