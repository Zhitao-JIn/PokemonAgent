...
> (2) **`sequence` 长度大于 1 时不再只允许 `up`/`down`**——(14) 起每段是四个方向键之一、
> **最后一段允许是 `a`**（最多一个、且只能在末尾），`a` 的 `times` 仍恒为 1。
> 同一次还多了**第 4 条硬规则**：rationale 只写结论、不写写出它的过程（改动动机是一条真机
> 记录——模型把「这一段立不住……修正：只保留一段」写进了 rationale，那几句话会作为该段的
> 论据永久进记忆），逐字理由与实测数字见 `CHANGELOG.md` 2026-09-12（23）。
